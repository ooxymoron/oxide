...
pub type TargetData = (Vector3, &'static mut Entity, HitboxId, isize, isize);
...
impl Aimbot {
    ...
    pub fn find_target(&self) -> OxideResult<Option<TargetData>> {
        let mut target = self.find_player()?;

        if setting!(aimbot, target_sentries) {
            if let Some((point, ent, hitbox_id, prio, point_prio)) = self.find_sentry()? {
                if let Some((_, _, _, last_prio, last_point_prio)) = &target {
                    if prio > *last_prio || (prio == *last_prio && *last_point_prio < point_prio) {
                        target = Some((point, ent, hitbox_id, prio, point_prio));
                    }
                } else {
                    target = Some((point, ent, hitbox_id, prio, point_prio));
                };
    ...
    pub fn create_move(&mut self, cmd: &mut UserCmd) -> OxideResult<()> {
        ...
        if let Some((point, ent, _, _, _)) = self.find_target()? {
            let p_local = &*Entity::get_local().unwrap();
            let my_eyes = vmt_call!(p_local.as_ent(), eye_position);
            let diff = my_eyes - point;
            let angle = diff.angle();
            if setting!(aimbot, autoshoot) {
                if self.shoot(cmd, Some(ent)) {
    ...
    pub fn shoot(&mut self, cmd: &mut UserCmd, found: Option<&Entity>) -> bool {
        let p_local = &*Entity::get_local().unwrap();
        ...
        let class = found.as_networkable().get_client_class().class_id;
        if matches!(
        ...
        ) && setting!(aimbot, ambasador_wait_for_hs)
        {
            let baim_lethal = (setting!(aimbot, baim_if_lethal)
                && vmt_call!(weapon.as_gun(), get_projectile_damage) >= (vmt_call!(found, get_health)) as f32);
            let shoot = o!().global_vars().time - weapon.last_fire > 1.0
                || baim_lethal
            ...
            | WeaponType::SniperrifleClassic => {
                let baim_lethal = (setting!(aimbot, baim_if_lethal)
                    && vmt_call!(weapon.as_gun(), get_projectile_damage)
                        >= (vmt_call!(found, get_health)) as f32);
                let classic = matches!(id, WeaponType::SniperrifleClassic);
                if !p_local.condition.get(ConditionFlags::Zoomed) && !classic && !baim_lethal {
