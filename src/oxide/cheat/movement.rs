...
impl Movement {
    ...
    pub fn create_move(&mut self, cmd: &mut UserCmd, org_cmd: &UserCmd) -> OxideResult<()> {
        ...
        if p_local.as_ent().flags.get(Flag::INWATER)
            || p_local.as_ent().flags.get(Flag::SWIM)
            || p_local.as_ent().water_level > WaterLevel::Feet
        {
            return Ok(());
        }
        let on_ground = p_local.as_ent().flags.get(Flag::ONGROUND);
        let jumping = cmd.buttons.get(ButtonFlags::InJump);
        if on_ground {
            self.double_jumped = false
        } else if jumping
            && matches!(p_local.player_class, PlayerClass::Scout)
            && !self.double_jumped
    ...
    pub fn auto_strafe(&self, cmd: &mut UserCmd) -> OxideResult<()> {
        let p_local = Entity::get_local()?;
        if p_local.as_ent().flags.get(Flag::ONGROUND) || !setting!(movement,autostrafe)
        {
            return Ok(());
        }
